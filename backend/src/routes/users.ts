...
router.put('/change-email', auth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { newEmail, currentPassword } = req.body;
    
    if (!newEmail || !currentPassword) {
      res.status(400).json({ message: 'New email and current password are required' });
      return;
    }
...
router.put('/change-password', auth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
